...
class PETRTransformer(BaseModule):
    ...
    def forward(self, x, mask, query_embed, pos_embed, reg_branch=None):
        ...
        memory = x.permute(1, 3, 0, 2).reshape(-1, bs, c)
        # pos_embed = pos_embed.permute(1, 3, 4, 0, 2).reshape(-1, bs, c) # [bs, n, c, h, w] -> [n*h*w, bs, c]
        pos_embed = pos_embed.permute(1, 3, 0, 2).reshape(-1, bs, c)
    ...
    def forward(self, query, *args, **kwargs):
        ...
        for l, layer in enumerate(self.layers):
            if self.use_intermediate_feat:
                query = layer(query, *args, key=key[l], value=value[l], **kwargs)
            else:
                query = layer(query, *args, key=key, value=value, **kwargs)
